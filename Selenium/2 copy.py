...
# days = soup.find_all('p', 'schedule__day__content__header__dayweek')
# content_lesons = soup.find_all("div", "columns")
# days = soup.find_all("schedule__day__content__lesson  schedule__day__content__lesson--main")


for i in container:
    class_data[i.find("div", "column-40").text.strip()] = {}
    # lessons = soup.find_all("schedule__day__content__lesson  schedule__day__content__lesson--main")
    # print(lessons)
    lessons = i.find_all("div", "schedule__day__content__lesson  schedule__day__content__lesson--main")

    
    for j in range(len(lessons)):
        class_data[i.find("div", "column-40").text.strip()][j.find("div", "schedule__day__content__lesson__num").text.strip()] = lessons[j].text.strip()
        print(i.find("div", "column-40").text.strip(), j.find("div", "schedule__day__content__lesson__num").text.strip())
