...
container = soup.find_all("div", "d-track typo-track d-track_selectable d-track_with-cover d-track_with-chart")
...
for i in container:
    num += 1
    name = i.find("div", "d-track__name").text
    name = name.strip()
    track = i.find("div", "d-track__meta").text
    #print(num, 'Трэк: ', name, 'Исполнитель:', track)
    chart_data.append({
            '№': num,
            'Трек': name,
            'Исполнитель': track
